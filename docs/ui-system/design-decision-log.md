...
| Plain suggested-tags field | InputWithTags | Contributors should add tags from existing visible-content and TJC vocabulary without creating a second taxonomy | Suggestion buttons and typed canonical matches use `canonicalTags`; non-canonical typed terms are rejected with intake-note guidance; final ResourceSpace taxonomy still requires reviewer approval |
