...
|---|---|---|---|---|---|---|
| Tubelight-style workflow nav | Implemented | Flat stretched top row with equal-weight controls | Gives Library, Collections, Upload, and Review clear workflow orientation while keeping Command, Guide, role, and avatar secondary | Centered floating pill, evergreen/aqua lamp line, active glow, compact laptop spacing, mobile bottom nav, no heavy glass | Keep `aria-current`; 768px and below use top brand/menu plus bottom nav; icon-only desktop utilities stay named | `frontend/components/AppNav.tsx`, `frontend/components/AppChrome.tsx` |
| Command palette / action search | Implemented | Slow manual route/search jumps | Speeds saved views, collection navigation, ResourceSpace ID lookup, Upload, stable Review queue URLs, Guide, Admin | Header trigger opens a keyboard-first DAM launcher with Go to / Find / Saved views / Workflow groups, large search, left icons, descriptions, shortcut chips, selected aqua row, and footer safety copy | Arrow/Home/End selection, `aria-activedescendant`, focus trap, Escape close, return-focus behavior, dynamic RS ID search, and role-aware Admin visibility are implemented | `frontend/components/CommandPalette.tsx`, `frontend/components/AppChrome.tsx`, `frontend/components/ReviewPage.tsx`, `frontend/app/review/page.tsx` |
