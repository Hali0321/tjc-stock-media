...
| Input with tags | Implemented | Plain comma-only suggested-tags field | Suggested tags should be easy to add while staying tied to existing taxonomy/export terms | Compact chips, suggestion buttons, evergreen focus, hidden canonical-only serialized form value | Uses `canonicalTags` only; typed non-canonical terms are rejected with guidance to add new wording to intake notes; reviewers still approve final taxonomy before ResourceSpace write | `frontend/components/InputWithTags.tsx`, `frontend/components/UploadPage.tsx`, `frontend/lib/taxonomy.ts`, `frontend/app/api/upload/route.ts` |
