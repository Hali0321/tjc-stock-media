...
| Command palette / action search | Implemented | Slow manual route/search jumps | Speeds saved views, collection navigation, ResourceSpace ID lookup, Upload, stable Review queue URLs, Guide, Admin | Header trigger now behaves like a compact action-search control; dialog uses a labelled search field, evergreen action rows, selected-row ring, and no neon/marketing styling | Arrow/Home/End selection, focus trap, Escape close, return-focus behavior, and queue URL state are implemented | `frontend/components/CommandPalette.tsx`, `frontend/components/AppChrome.tsx`, `frontend/components/ReviewPage.tsx`, `frontend/app/review/page.tsx` |
