...
export function CollectionShelfInspector({ collection, totalCollections, onOpen }: CollectionShelfInspectorProps) {
      ...
      <section className="grid min-h-64 place-items-center rounded-md border border-[#d4ded7] bg-white p-4 text-center" aria-label="Selected collection">
        <div className="grid max-w-xs justify-items-center gap-2">
          ...
          <p className="text-sm leading-relaxed text-tjc-muted">
            Collection context loads from the same ResourceSpace export as Library search.
          </p>
        </div>
      </section>
    );
  ...
  return (
    <section className="overflow-hidden rounded-[1.45rem] border border-[#d4ded7] bg-white shadow-[0_18px_48px_rgba(25,34,29,.06)]" aria-label={`${collection.name} collection inspector`}>
      <div className="grid gap-4">
        <div className="relative overflow-hidden bg-[#f6f8f5]" aria-hidden="true">
          ...
          <span className="absolute left-4 top-4 rounded-full border border-white/80 bg-white/86 px-3 py-1 text-xs font-black text-tjc-evergreen shadow-[0_12px_28px_rgba(25,34,29,.10)]">
            Selected collection
          </span>
        </div>
        <div className="px-4">
        ...
        <div className="grid gap-2 px-4 sm:grid-cols-2">
          {[
            ...
            return (
              <div className="rounded-2xl border border-[#dbe5de] bg-[#fbfcfa] p-3 shadow-[0_1px_0_rgba(255,255,255,.9)_inset]" key={item.label}>
        ...
        <div className={hasPeopleWarning ? "mx-4 border-l-2 border-[#c9922e] pl-3 text-[#725216]" : "mx-4 border-l-2 border-[#7db58f] pl-3 text-[#22563a]"}>
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <span className="mt-1 block text-xs leading-relaxed">{hasAssets ? "Confirm per-asset reuse in Library before publication." : "It will appear in Library when assets are approved."}</span>
            </div>
          </div>
        </div>
        ...
        {hasAssets ? (
          <button className="mx-4 mb-4 dam-button-primary inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition active:translate-y-px" type="button" onClick={() => onOpen(collection)}>
            Open Library results
            <ArrowUpRight size={16} strokeWidth={1.8} aria-hidden="true" />
          </button>
        ) : (
          <div className="mx-4 mb-4 rounded-xl border border-[#d6dfd8] bg-[#fbfcfa] p-3 text-sm font-black text-tjc-muted">
            No assets yet
          </div>
        )}
      </div>
