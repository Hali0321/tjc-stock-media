...
export function ImageComparisonPanel({ safePreview, alt, approvedCopyAllowed, originalHidden }: ImageComparisonPanelProps) {
  ...
  return (
    <section className="rounded-[1.35rem] border border-[#d4ded7] bg-white p-3" aria-label="Derivative comparison">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <span className={cn("rounded-full border px-2.5 py-1 text-xs font-black", approvedCopyAllowed ? "border-[#b8d9c6] bg-[#edf8f1] text-[#22563a]" : "border-[#ead6a8] bg-[#fff8e8] text-[#725216]")}>
          {approvedCopyAllowed ? "Approved copy available" : "Copy blocked"}
        </span>
      </div>
      <div className="relative overflow-hidden rounded-[1.1rem] border border-[#dbe4dd] bg-[#f5f7f4]">
        ...
        {safePreview ? (
          <div className="pointer-events-none absolute inset-0 border-r-2 border-white/90 bg-white/10" style={{ clipPath: `inset(0 ${100 - position}% 0 0)` }} aria-hidden="true">
            <MediaPreview src={safePreview} alt="" imgClassName="h-full w-full object-contain saturate-[.92]" className="px-4" />
          </div>
        ) : null}
        <div className="absolute left-3 top-3 rounded-full bg-white/92 px-3 py-1 text-xs font-black text-tjc-evergreen shadow-sm">Display derivative</div>
