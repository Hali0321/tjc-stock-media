        ...
        return (
          <Link href={`/assets/${asset.id}`} key={asset.id} className="group overflow-hidden rounded-lg border border-tjc-line bg-white transition hover:-translate-y-0.5 active:translate-y-px">
            <span className="grid aspect-[4/3] place-items-center bg-[#e8eee8] p-2">
              {imageUrl ? (
                <img className="h-auto max-h-36 w-auto max-w-full rounded-md object-contain shadow-[0_4px_12px_rgba(32,34,31,.12)] transition duration-500 group-hover:scale-[1.025]" src={imageUrl} alt={asset.thumbnailAlt} loading="lazy" />
              ) : (
                <span className="rounded-md border border-tjc-line bg-white/70 px-2 py-1 text-center text-xs font-semibold text-tjc-muted">Preview restricted</span>
              )}
            </span>
      ...
      <section className="mt-4 grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(390px,.85fr)]">
        <div className="min-w-0">
          ...
          <section className="mt-4 rounded-lg border border-tjc-line bg-white/76 p-4" aria-label="Related assets">
            <div className="mb-3">
              <h2 className="text-lg font-semibold">Related assets</h2>
        ...
        <aside className="grid min-w-0 gap-3 xl:sticky xl:top-24 xl:self-start">
          <section className="min-w-0 rounded-lg border border-tjc-line bg-white/82 p-4">
            <span className="text-sm font-semibold text-tjc-evergreen">{asset.collection}</span>
            <h1 className="mt-2 text-3xl font-semibold leading-tight md:text-4xl">{display.title}</h1>
            <p className="mt-2 text-sm leading-relaxed text-tjc-muted">{provenance.publicLabel}</p>
          </section>

          <AssetTrustPanel asset={asset} role={role} />
          <section className="min-w-0 rounded-lg border border-tjc-line bg-white/82 p-4" aria-label="Governance passport">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h2 className="flex items-center gap-2 text-lg font-semibold"><ShieldCheck size={18} strokeWidth={1.8} aria-hidden="true" /> Governance passport</h2>
          ...
          {activeTab === "Use" ? (
            <section className="min-w-0 rounded-lg border border-tjc-line bg-white/82 p-4" aria-label="Usage guidance">
          ...
          {activeTab === "Source" ? (
            <section className="min-w-0 rounded-lg border border-tjc-line bg-white/82 p-4" aria-label="Source and provenance">
          ...
          {activeTab === "Review" ? (
            <section className="min-w-0 rounded-lg border border-tjc-line bg-white/82 p-4" aria-label="Review status">
          ...
          {activeTab === "Files" ? (
            <section className="min-w-0 rounded-lg border border-tjc-line bg-white/82 p-4" aria-label="File options">
          ...
          {activeTab === "Related" ? (
            <section className="min-w-0 rounded-lg border border-tjc-line bg-white/82 p-4" aria-label="Related assets">
          ...
          <section className="min-w-0 rounded-lg border border-tjc-line bg-white/82 p-4" aria-label="Tags">
