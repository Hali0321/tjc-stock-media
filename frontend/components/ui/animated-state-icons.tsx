...
export function CopiedIcon({ size = 40, color = "currentColor", className, duration = 2200 }: StateIconProps) {
  ...
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn(className)} style={{ width: size, height: size }}>
      ...
      <path d="M10 14a2 2 0 00-2 2v18a2 2 0 002 2h14" stroke={color} strokeWidth={2} strokeLinecap="round" opacity={0.3} />
      <AnimatePresence mode="wait">
        {copied ? (
          <motion.path
            key="check"
...
export function NotificationIcon({ size = 40, color = "currentColor", className, duration = 2800 }: StateIconProps) {
  const notification = useAutoToggle(duration);
  ...
  return (
    <motion.svg
      ...
      animate={notification ? { rotate: [0, 8, -8, 6, -6, 3, 0] } : { rotate: 0 }}
    ...
    >
      ...
      <motion.circle
        ...
        r="4"
        fill="#EF4444"
        animate={notification ? { scale: [0, 1.3, 1], opacity: 1 } : { scale: 0, opacity: 0 }}
        transition={{ duration: 0.4, ease: [0.32, 0.72, 0, 1] }}
      />
    </motion.svg>
  );
    ...
    <svg viewBox="0 0 40 40" fill="none" className={cn(className)} style={{ width: size, height: size }}>
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.g key="arrow" initial={{ y: -4, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 8, opacity: 0 }} transition={{ duration: 0.35, ease: [0.32, 0.72, 0, 1] }}>
    ...
    <div className="mx-auto w-full max-w-3xl px-4 py-16">
      <div className="mb-14 text-center">
        ...
        <p className="mx-auto max-w-md text-sm text-muted-foreground">
          Twelve icons that morph between two meaningful states on loop.
        </p>
      </div>
