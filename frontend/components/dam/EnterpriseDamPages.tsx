  ...
  return (
    <div className="enterprise-page enterprise-insights">
      ...
      {insights.loading ? <LoadingCard /> : insights.error ? <ErrorCard message={insights.error} source={insights.source} /> : <>
        ...
        <div className="ed-insights-grid">{insightCharts.map((chart) => <ChartCard key={chart.id} title={chart.title} large={chart.large} sample={chart.sample}>{chart.id === "top-assets" ? (insights.data?.assets || []).map((asset, i) => <p className="ed-top-asset" key={asset.id}><span>{i + 1}</span><AssetThumb asset={asset} /><strong title={displayTitle(asset)}>{displayTitle(asset)}</strong><small>{asset.resourceSpaceId || asset.id}</small></p>) : chart.rows ? <div className="ed-table-mini">{chart.rows.map((row) => <p key={row}>{row}</p>)}</div> : <div className="ed-big-line"><MiniLine /><MiniLine tone={chart.tone || "indigo"} /></div>}</ChartCard>)}</div>
