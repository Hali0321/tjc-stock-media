...
function AuditTable({ readiness }: { readiness?: DamReadinessResult | null }) {
  return (
    <section className="ed-card">
      <header className="ed-card-head"><h3>Recent Audit Activity</h3><ActionButton>View all logs</ActionButton></header>
      <table className="ed-table">
...
function OverviewModule({ readiness }: { readiness?: DamReadinessResult | null }) {
  return (
    <>
      ...
      <AuditTable readiness={readiness} />
    </>
  );
}

function AdminModuleContent({ activeNav, readiness }: { activeNav: string; readiness?: DamReadinessResult | null }) {
  const metrics = readiness?.metrics;
  const integrations = readiness?.integrationReadiness || [];
  if (activeNav === "overview") return <OverviewModule readiness={readiness} />;
      ...
      <div className="ed-admin-grid">
        ...
        <main>
          ...
          {admin.loading ? <LoadingCard /> : admin.error ? <ErrorCard message={admin.error} source={admin.source} /> : <>
            <AdminModuleContent activeNav={activeNav} readiness={readiness} />
          </>}
        </main>
