...
export function EnterpriseBrandHubPage() {
  ...
  const focusShare = () => {
    ...
    setKitMessage("Share panel opened. Invite stays local in this beta until collaboration backend is connected.");
  };
  return (
    <div className="enterprise-page enterprise-brand">
      <div className="ed-brand-top"><div className="ed-breadcrumb">Brand Hub <span>›</span> Ministry Kits <span>›</span> MVP 2024</div><div><ActionButton icon={Star} onClick={() => setKitMessage("MVP 2024 saved to this beta session.")}>Save</ActionButton><ActionButton icon={Share2} onClick={focusShare}>Share Kit</ActionButton><ActionButton tone="dark" icon={Download} onClick={() => setKitMessage(connected ? "Package ZIP export is not enabled yet. Download individual ResourceSpace-backed assets below." : "Download disabled until this kit is connected to a ResourceSpace collection.")}>Download Kit</ActionButton><div className="ed-action-menu-wrap"><IconButton label="More" onClick={() => setMoreOpen((open) => !open)}><MoreHorizontal size={16} /></IconButton>{moreOpen ? <div className="ed-more-actions-menu ed-brand-more" role="menu"><button type="button" onClick={configureCollection}>View setup details<span>{role === "DAM Admin" ? "Show server env key and collection status." : "Admin-only ResourceSpace setup."}</span></button><button type="button" onClick={() => { navigator.clipboard?.writeText(window.location.href); setKitMessage("Kit link copied."); setMoreOpen(false); }}>Copy kit link<span>Copies this Brand Hub URL.</span></button><a href="/guide" onClick={() => setMoreOpen(false)}>Open help guide<span>Role-safe DAM guidance.</span></a></div> : null}</div></div></div>
