...
export function EnterpriseBrandHubPage() {
  ...
  const nextSafeAction = governance?.canDownloadKit
    ? { title: "Kit ready for download gate", detail: "Every mapped asset is Portal Ready. Source records remain canonical in the DAM.", action: "Prepare kit packet", tone: "ready" }
    : governance?.configured
      ? { title: "Review blockers before delivery", detail: governance?.blockers[0] || "Mapped assets need more review before download or broad sharing.", action: "View blockers", tone: "review" }
      : { title: "Connect ResourceSpace collection first", detail: "No downloadable kit is shown until this Brand Hub maps to real DAM records.", action: role === "DAM Admin" ? "View setup details" : "Ask DAM Admin", tone: "blocked" };
