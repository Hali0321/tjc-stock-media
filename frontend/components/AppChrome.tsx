      ...
      <header className="sticky top-0 z-40 border-b border-tjc-line bg-[#f8faf6]/94 backdrop-blur-xl">
        <div className="mx-auto grid min-h-16 w-full max-w-[1760px] gap-2 px-3 py-2 md:grid-cols-[auto_1fr_auto] md:items-center md:px-5">
            ...
            {nav.filter((item) => !("adminOnly" in item) || role === "DAM Admin").map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={cn(
                    "inline-flex min-h-9 items-center gap-2 rounded-lg px-3 text-sm font-semibold text-[#3f4741] transition hover:bg-white hover:text-tjc-evergreen active:translate-y-px",
          ...
          <div className="flex min-w-0 items-center justify-end gap-2">
            <Link href="/guide" className="hidden min-h-9 items-center gap-2 rounded-lg border border-tjc-line bg-white px-3 text-sm font-semibold text-tjc-evergreen transition hover:bg-[#eef5f1] active:translate-y-px md:inline-flex" aria-label="Open usage guide">
              <HelpCircle aria-hidden="true" size={16} strokeWidth={1.8} />
              <span>Guide</span>
            </Link>
            {role === "DAM Admin" ? (
              <a className="hidden min-h-9 items-center gap-2 rounded-lg border border-tjc-line bg-white px-3 text-sm font-semibold text-tjc-evergreen transition hover:bg-[#eef5f1] active:translate-y-px lg:inline-flex" href="http://localhost:8088" target="_blank" rel="noreferrer">
